#' Amino Acid/Nucleotide Diversity (Index Pi)
#'
...
#'
#' @details Nucleotide diversity (pi) is a genetic diversity index that measures the average Hamming distance
...
#'
#' @return Returns the nucleotide or amino acid diversity as a numeric value.
#'
#' @references
...
#' pi(seqs, count, weighted)
#'
#' @seealso See \code{\link[Biostrings]{stringDist}} in \strong{Biostrings} for the function that calculates the distance scores.
#'
#' @import Biostrings
#'
#' @export
pi <- function(seqs, count, weighted)
{
  n <- sum(count)
  if(n<2) return(0)
  ...
  if (weighted==TRUE){
    ...
    p <- rep(1/length(count), length(count)) # unweighted
  }
  dst <- stringDist(seqs, method="hamming", diag=TRUE, upper=TRUE)
  D <- as.matrix(dst)
  (n/(n-1)) * (t(p) %*% D %*% p)[1]
}
